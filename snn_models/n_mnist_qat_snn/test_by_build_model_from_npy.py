...
#from load_from_model_params import *
...
#for idx, layer in enumerate(snn_quant.linear_layers):
    ...
    ## Read from npu subdirectories
    #weight_filepath = model_dir / Path("npu") / model_params_dir / Path(params_file_basename+str(idx)+"_weights.npy")
    #bias_filepath = model_dir / Path("npu") / model_params_dir / Path(params_file_basename+str(idx)+"_biases.npy")


    #weight = np.load(weight_filepath)
    #bias = np.load(bias_filepath)

    #print("weight file:", weight.shape, "expected:", layer.weight().shape)
    #print("bias file:", bias.shape, "expected:", layer.bias().shape)

    ##print("layer.weight:", layer.weight.__class__)
    ##print("layer.bias:", layer.bias.__class__)
    ##with torch.no_grad():
        ##layer.weight().data.copy_(torch.from_numpy(weight).to(layer.weight().dtype))
        ##layer.bias.data.copy_(torch.from_numpy(bias).to(layer.bias.dtype))

    ## Quantize weights to match existing layer
    ...
    ## Build a new quantized Linear with the same parameters
    #new_layer = torch.nn.quantized.Linear(
    ...
    ## Assign the packed params safely
    #new_layer.set_weight_bias(w=q_weight, b=torch.from_numpy(bias))
...
test_data = np.load(model_dir / Path("npu") / test_patterns_dir / Path("test_input_0.npy"))
target_data = np.load(model_dir / Path("npu") / test_patterns_dir / Path("test_target_0.npy"))
