...
#include "../include/ethosu_driver.h"
...
extern double avg_inference_time_ethosu_invoke_v3;
/******************************************************************************
 * Types
 ******************************************************************************/
...
static int handle_command_stream(struct ethosu_driver *const drv, const uint8_t *cmd_stream, const int cms_length)
{
                ...
                uint32_t privilege_enable)
{
    ...
    //mydebug
    if (DEBUG_MODE) {
        printf("ETHOSU_INIT IS BEING CALLEDD!!!!!!!\n\n\n\n\n\n\n\n");
        printf("Initializing NPU: base_address=%p, fast_memory=%p, fast_memory_size=%zu, secure=%" PRIu32
        ", privileged=%" PRIu32 "\n",
        base_address,
        fast_memory,
        fast_memory_size,
...
int ethosu_wait(struct ethosu_driver *const drv, bool block)
{

    //mydebug
    if (DEBUG_MODE) {
        printf("\t block = %d", block);
        printf("In ethosu_wait: drv->job.state = %d\n",drv->job.state);
    }
                        ...
                        void *user_arg)
{
    ...
    //mydebug
    if (DEBUG_MODE) {
        printf("in ethosu_invoke_async:\n");
        for (int i = 0; i < num_base_addr; i++) {
            printf("base_addr[%d] = %p\n", i, base_addr[i]);
        }
    }
                     ...
                     void *user_arg)
{
    ...
    //mydebug
    if (DEBUG_MODE) {
        printf("in ethosu_invoke_v3:\n");
        for (int i = 0; i < num_base_addr; i++) {
            printf("base_addr[%d] = %p\n", i, base_addr[i]);
        }
        for (int i = 0; i < num_base_addr; i++) {
            printf("base_addr_size[%d] = %d\n", i, base_addr_size[i]);
        }
    }
    
    printf("CONGRATS WE ARE IN THE RIGHT FILE!!!\n");
    ...
    //mydebug
    //printf("about to enter ethosu_wait\n");


    //return ethosu_wait(drv, true);

    int tmp = ethosu_wait(drv, true);

    // Stop timer
    uint32_t inference_speed_measure_elapsed_ticks = debug_end_timer(inference_speed_measure_start_tick);
    avg_inference_time_ethosu_invoke_v3 += inference_speed_measure_elapsed_ticks;
