param (
    ...
    [int]$InputNeuronsStartRange = 16,
    [int]$InputNeuronsEndRange = 512+16,
...
for ($inputNeurons = $InputNeuronsStartRange; $inputNeurons -le $InputNeuronsEndRange; $inputNeurons += 16) {
    for ($outputNeurons = $outputNeuronsStartRange; $outputNeurons -le $outputNeuronsEndRange; $outputNeurons += 16) {

        Write-Host "Processing with inputNeurons = $inputNeurons,   outputNeurons = $outputNeurons" -ForegroundColor Green
