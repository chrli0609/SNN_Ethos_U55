...
def test_patterns_2_h_file(num_samples, mem_section_name, input_file_path, target_file_path, output_file_path):
    ...
    try:
        ...
        # Squeeze axis 2 (just like PyTorch's squeeze(2))
        test_input_untransposed = np.squeeze(test_input_untransposed, axis=2)  # Removes axis 2 if its size is 1

        # Flatten each element in the batch (same as torch.flatten(x, 1))
        flattened = [x.reshape(x.shape[0], -1) if x.ndim > 1 else x for x in test_input_untransposed]

        print("Transpose test_inputs to get samples on first axis instead of second")
        test_input_unstripped = np.transpose(flattened, (1, 0, 2))  # Rearrange axes
