...
def cms_bytearr_2_assembly(cms_byte_arr):

    for i in range(len(cms_byte_arr)):
        num_elements = len(cms_byte_arr[i])
        if num_elements == 1:
            has_payload = False
        elif num_elements == 2:
            has_payload = True
            payload = cms_byte_arr[i][1]
        else:
            print("Three elements found in tuple")
            exit()


        command = cms_byte_arr[i][0]

        bin_cmd = bin(command)[2:].zfill(32)

        cmd = bin_cmd[:10]
        parameter = bin_cmd[16:]

        if bin_cmd[10:16] == '000000': #cmd0 
            cmd_name = cmd0_dict[int(cmd, 2)]
        elif bin_cmd[10:16] == '010000': #cmd1
        ...
        print(cmd_name + "\t" + format(parameter, '08x'), end='')
        if has_payload:
            print("\t\t" + format(has_payload, '08x'))
        else:
            print()
